...
trace_reader* init(trace_sim_args* tsa)
{
    ...
    int op = 0;
    while ((op = getopt(tsa->arg_count, tsa->arg_list, "hdvc:p:o:n:b:t:s:")) != -1)
    {
        switch (op)
        {
            case 't':
...
trace_op* getNextOp(int processorNum)
{
    ...
    switch (opType)
    {
        case 'A':
            ...
            {
                free(op);   
                return getNextOp(processorNum);
            }
            
            break;
        case 'B':
            op->op = BRANCH;
            fscanf(tf, "%lx %lx", &pcAddress, &nextPC);
            if (1 == fscanf(tf, " %d\n", &op0))
            {
                op->src_reg[0] = op0;
            }
        ...
        case 'L':
            op->op = MEM_LOAD;
            fscanf(tf, "%lx,%d", &memAddress, &opSize);
            if (1 == fscanf(tf, " %d\n", &op0))
            {
                op->src_reg[0] = op0;
            }
...
int is_eof(int processorNum) {
    return feof(traceFile[processorNum]);
}
