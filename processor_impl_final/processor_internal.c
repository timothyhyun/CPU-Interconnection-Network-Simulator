...
// Data collected for instrumentation
uint64_t CYCLES_DISP_Q_1_MAXED = 0;
uint64_t CYCLES_DISP_Q_2_MAXED = 0;
/*
  new->funcUnit0 = calloc(sizeof(shortALU_t), j);
  new->funcUnit1 = calloc(sizeof(longALU_t), k);
*/
uint64_t *CYCLES_ALU_0_IDLE = NULL;
uint64_t *CYCLES_ALU_1_IDLE = NULL;
...
void ins_print(void *data)
{
  ...
  printf("\n{tag: %i, op: %i, thread: %i} ", dat->tag, dat->op, dat->thread);
}
...
processor_t *init_p(size_t D, size_t F, size_t m, size_t j, size_t k, size_t c)
{
  ...
  CYCLES_ALU_0_IDLE = calloc(sizeof(uint64_t), j);
  CYCLES_ALU_1_IDLE = calloc(sizeof(uint64_t), k);
  ...
  new->params->k = k;
  new->params->c = c;
  new->disp_queue[0] = init_q(D * (m * j + m * k));
  new->disp_queue[1] = init_q(D * (m * j + m * k));
  ...
  new->fetch_unit_go[0] = true;
  new->fetch_unit_go[1] = true;
  ...
  for (int i = 0; i < 2; i++)
  {
    for (int j = 0; j < 33; j++)
    {
      new->registers[i][j].tag = 0;
      new->registers[i][j].busy = false;
    }
  }
  return new;
}
...
bool dispatch_full(processor_t *P, int idx)
{
  ...
  return P->disp_queue[idx]->capacity == 0;
}
...
void p_register_update(processor_t *P)
{
  ...
  for (int i = 0; i < P->params->c; i++)
  {
    ...
    {
      int dest_proc = P->resBusses[i].dest_proc;
      int dest = P->resBusses[i].dest;
      int tag = P->resBusses[i].tag;
      assert(dest_proc >= 0 && dest_proc < 2);
      assert(dest >= 0 && dest_proc < 33);
      // Should only update dest registers if the tag is current
      if (dest >= 0 && P->registers[dest_proc][dest].tag == tag)
      {
        P->registers[dest_proc][dest].busy = false;
      }
    }
  }
}
...
void p_dispatch_reserve(processor_t *P)
{
  ...
  // This block of code gives all of the instructions in the dispatch queue
  // the stage in which they enter dispatch
  for (int i = 0; i < 2; i++)
  {
    list_t *pass1 = P->disp_queue[i]->q;
    while (pass1 != NULL)
    {
  ...
  int curr_thread = P->sche_thread;
  
  // If the thread we are due to fetch 
  if (P->disp_queue[curr_thread]->q == NULL) {
    curr_thread = (curr_thread + 1) % 2;
  }

  // This block of code actually moves ready instructions into the scheduler
  // We need the queue capacities (and update them locally) in order to simulate
  // "Saving" the spots on the schedulers
  list_t *curr = P->disp_queue[curr_thread]->q;
  while (curr != NULL)
  {
    ...
    case MEM_STORE:
      ...
      if (!go)
      {
        P->sche_thread = (P->sche_thread + 1) % 2;
        return;
      }
      else
      {
    ...
    list_t *old = curr;
    curr = curr->next;
    delete (P->disp_queue[curr_thread], old);
  }
  P->sche_thread = (P->sche_thread + 1) % 2;
}
...
void p_defer(processor_t *P)
{
  ...
  while ((curr_ins = (instruction_t *)dq(P->defer_queue)))
  {
    ...
    // Figure out if the registers are ready or not
    // Also update them to make sure they are set to busy when they are busy
    int proc = curr_ins->thread;
    for (int i = 0; i < 2; i++)
    {
      assert(proc >= 0 && proc < 2);
      if (!P->registers[proc][curr_ins->src_reg[i]].busy)
      {
        curr_ins->ready[i] = true;
      }
      else
      {
        curr_ins->src_tag[i] = P->registers[proc][curr_ins->src_reg[i]].tag;
        curr_ins->ready[i] = false;
      }
    }
    if (curr_ins->dest_reg >= 0)
    {
      P->registers[proc][curr_ins->dest_reg].tag = curr_ins->tag;
      P->registers[proc][curr_ins->dest_reg].busy = true;
    }
  }
}
...
void p_state_update(processor_t *P)
{
  ...
  for (int i = 0; i < P->params->c; i++)
  {
    ...
    if (!curr_bus->busy)
    {
      ...
      if (curr_ins == NULL)
      {
        printf("NULL instruction encounted when servicing busQueue\n");
        return;
      }
      switch (curr_ins->op)
      {
      ...
      case BRANCH:
        ...
        {
          P->fetch_unit_go[curr_ins->thread] = true;
        }
      case ALU:
        delete (P->sche_queue_0, curr_dat);
        break;
...
void instrument(processor_t *P) {
  // Collect data on ALU usage, dispatch queue fullness
  for (int i = 0; i < P->params->j; i++) {
    if(P->funcUnit0[i].stage1 == NULL) {
      CYCLES_ALU_0_IDLE[i]++;
    }
  ...
  for (int i = 0; i < P->params->k; i++) {
    if(P->funcUnit1[i].stage1 == NULL && P->funcUnit1[i].stage2 == NULL && P->funcUnit1[i].stage3 == NULL) {
      CYCLES_ALU_1_IDLE[i]++;
    }
  }
}

void print_data(processor_t *P) {
  printf("Cycles Type 0 function units idle:\n");
  for (int i = 0; i < P->params->j; i++) {
    printf("Type 0 ALU #%d: idle for %ld\n", i, CYCLES_ALU_0_IDLE[i]);
  }
  printf("Cycles Type 1 function units idle:\n");
  for (int i = 0; i < P->params->k; i++) {
    printf("Type 1 ALU #%d: idle for %ld\n", i, CYCLES_ALU_1_IDLE[i]);
  }
}
...
{
  printf("Cycle %li: \n", CURR_CYCLE);
  printf("Dispatch queue 0: ");
  print_queue(P->disp_queue[0], &ins_print);
  printf("Dispatch queue 1: ");
  print_queue(P->disp_queue[1], &ins_print);
...
void vbs_print(queue_t *Q)
{
  ...
  while (curr)
  {
    ...
    if (curr_ins->tag != CURR_PRINT_TAG || curr_ins->vdat->stage_su == -1 || !curr_ins->retire)
    {
      return;
    }
    curr = curr->next;
    dq(Q);
    printf("Thread %li: %li %li %li %li %li %li\n",
           curr_ins->thread, curr_ins->tag, curr_ins->vdat->stage_fetch,
