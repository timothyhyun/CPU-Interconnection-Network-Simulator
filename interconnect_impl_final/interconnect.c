...
const int MEM_WRITE_DELAY = 100; // Wait this long when we try to FLUSH the data
...
void busReq(bus_req_type brt, uint64_t addr, int procNum)
{
    ...
    // What is this part even supposed to do
    else if (brt == FLUSH && pendingRequest->addr == addr)
    {
        assert(pendingRequest->currentState == WAITING_MEMORY);
        countDown = MEM_WRITE_DELAY;
        return; 
    }
    else
    {
